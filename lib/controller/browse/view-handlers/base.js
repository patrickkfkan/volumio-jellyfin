...
class BaseViewHandler {
    ...
    _constructUriSegment(view, addToStartIndex = 0) {
        ...
        Object.keys(view).filter( key => key !== 'name' && key !== 'startIndex' && key !== 'serverId' && key !== 'saveFilter' ).forEach( (key) => {
            ...
            if (itemText) {
                ...
                <div style="width: 100%;">
                    <div style="display: flex; align-items: center; font-size: 14px; border-bottom: 1px dotted; border-color: #666; padding-bottom: 10px;">
                        ...
                        ${ parentText ? `<i class="fa fa-angle-right" style="margin: 0px 10px;"></i><span style="font-size: 18px;">${ parentText }</span>` : ''}
                    </div>
